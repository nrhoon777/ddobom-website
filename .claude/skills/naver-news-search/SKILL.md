...
description: '네이버 검색 Open API 뉴스 검색(news.json)을 k-skill-proxy 경유로 조회해 최신 뉴스 기사 제목·발행시각·링크·요약을 보수적으로 정리한다. 사용자는 별도 API 키 발급 없이 호출한다. [또봄] 언론 보도 모니터링, 뉴스 검색, 기사 클리핑, 단체·캠페인 노출 현황 확인에 사용.'
license: MIT
metadata:
...
이 스킬은 `.claude/skills/naver-news-search/` 에 통째로 복사되어 있다. Node.js/`npx` 가 없거나
