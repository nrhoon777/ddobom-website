---
name: hwp
description: 'Use kordoc for agent-native HWP/HWPX document parsing, JSON extraction, diffing, form-field extraction, and Markdown→HWPX reverse conversion (read/convert only — for binary editing use rhwp-edit). [또봄] 한글 문서(HWP/HWPX) 읽기, 텍스트·표 추출, 서식 필드 추출, 마크다운→HWPX 변환에 사용.'
license: MIT
metadata:
  category: documents
  locale: ko-KR
...
이 스킬은 `.claude/skills/hwp/` 에 통째로 복사되어 있다. Node.js/`npx` 가 없거나
